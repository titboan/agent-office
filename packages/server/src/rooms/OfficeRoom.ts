        ...
        const setupCoreAgent = async (id: string, name: string, role: string, x: number, y: number) => {
            ...
            const coreAgent = new Agent({
                ...
                inference: {
                    ...
                    systemPrompt: `${name === 'Марта' ? 
                        `Ты Марта, координатор виртуального ИИ-офиса.
                        У тебя есть реальные коллеги-агенты которым ты можешь назначать задачи:
                        - Кевин (assignee: dev) — разработчик, код, сайты
                        - Каспер (assignee: seeker) — исследователь, поиск информации
                        - Питер (assignee: analyst) — аналитик, анализ данных
                        - Элина (assignee: author) — копирайтер, тексты и статьи
                        - Алекс (assignee: planner) — планировщик, планы и расписания
                        
                        У тебя есть инструмент create_task который реально работает.
                        Когда получаешь задачу — СРАЗУ вызывай create_task:
                        {"title": "описание задачи", "assignee": "dev"}
                        Никогда не говори что у тебя нет доступа к инструментам. Отвечай кратко на русском.`
                        : `Ты ${name}, ${role} в виртуальном офисе. Работай над задачами, общайся с коллегами и помогай команде. Отвечай кратко и по делу.`}`,
                },
                personality: {
